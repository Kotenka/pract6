...
num_i_units = 2 # количество входных единиц
num_h_units = 2 # количество скрытых единиц измерения
num_o_units = 1 # количество выходных единиц

learning_rate = 0.1 # 0.001, 0.01 <- Magic values Скорость обучения для градиентного спуска.
reg_param = 0 # 0.001, 0.01 <- Magic values Параметр, помогающий при перенапряжении
max_iter = 5000 # 5000 <- Magic value максимальное колличество иттераций для градиентного спуска
...
# Weights1(подключение от входа к скрытым слоям):
# num_h_units X num_i_units
# Biases1 (подключение от входа к скрытым слоям): num_h_units X 1
# Weights2(соединение со скрытыми выходными слоями):
# num_o_units X num_h_units
# Biases2 (соединение со скрытыми выходными слоями): num_o_units X 1
...
def sigmoid(z, derv=False): #функция сигмоид
    if derv: return z * (1 - z) # если дерв true,
...
def forward(x, predict=False): # функция переадресации
    ...
    if predict: return a3 # если predict(прогноз) true,
...
for i in range(max_iter): # цикл фор от 0 до 5000
   #локальные переменные внутри цикла
    ...
    for j in range(m): # цикл внутри цикла от 0 до 4 m = 4 # Количество обучающих примеров
        ...
        # Вычислите ошибку в выходном слое (dz2).
        dz2 = a2 - y[j] # 1x1
        # Вычислите ошибку в Весах, соединяющих скрытый слой с выходным слоем, используя dz2 (dW2).
        ...
        # Вычислите ошибку в скрытом слое (dz1)
        ...
        # Вычислите ошибку в Весах, соединяющих входной слой со скрытым слоем, используя dz1 (dW1).
        dW1 += dz1.dot(a0.T) # 2x1 * 1x2 = 2x2

        # Ошибки в смещениях - это просто ошибки в соответствующих слоях.
        # Затем градиенты (ошибки) Весов и предубеждений используются для
        # обновления соответствующих Весов и предубеждений путем умножения
        # их на отрицательную величину скорости обучения и масштабирования ее
        # путем деления на количество обучающих примеров.
...
# функция forward принимает вектор объектов только как вектор строк,
# который затем преобразуется в вектор столбцов. Кроме того,
# логическое значение predict, если оно установлено в true,
# возвращает только выходные данные. В противном случае он
# возвращает кортеж выходных данных всех слоев.

for x in X:
    print("\n")
    print(x)
    print(forward(x, predict=True))
# Присвоение осей различным элементам.
plt.plot(range(max_iter), cost)
# Обозначение оси x как оси итераций.
plt.xlabel("Iterations")
# Обозначение оси y как оси затрат.
plt.ylabel("Cost")
# Показываем сюжет.
plt.show()
